(function () {
  ...
  var expandAfterNavigation = false;

  function expandSidebarToActive() {
    console.log('[Search] expandSidebarToActive called, flag:', expandAfterNavigation);
    if (!expandAfterNavigation) {
      return;
    }
    expandAfterNavigation = false;

    // 立即执行，在 sidebar-collapse-plugin 处理之前添加 active 类
    var hash = window.location.hash;
    console.log('[Search] Current hash:', hash);
    if (!hash) return;

    // 去掉查询参数（?id=...）
    var hashWithoutQuery = hash.split('?')[0];
    console.log('[Search] Hash without query:', hashWithoutQuery);

    // 查找侧边栏中对应的链接，尝试多种格式
    var sidebarLink = null;
    var attempts = [
      hashWithoutQuery,
      hashWithoutQuery.substring(1),
      decodeURIComponent(hashWithoutQuery),
      decodeURIComponent(hashWithoutQuery.substring(1)),
      hash,
      hash.substring(1),
    ...
    console.log('[Search] Attempting to find link with:', attempts);

    for (var i = 0; i < attempts.length; i++) {
      var links = Docsify.dom.findAll('.sidebar-nav a');
      for (var j = 0; j < links.length; j++) {
        var href = links[j].getAttribute('href');
        if (href === attempts[i] || decodeURIComponent(href) === attempts[i]) {
          sidebarLink = links[j];
          console.log('[Search] Found matching link:', href, 'at attempt', i);
          break;
        }
      }
    ...
    if (!sidebarLink) {
      console.log('[Search] No matching link found in sidebar');
      return;
    }

    // 找到包含该链接的 LI 元素
    var liNode = sidebarLink.parentNode;
    while (liNode && liNode.tagName !== 'LI') {
      liNode = liNode.parentNode;
    }

    if (liNode) {
      console.log('[Search] Found LI node:', liNode);

      // 先清除所有激活状态
      Docsify.dom.findAll('.sidebar-nav .active').forEach(function (el) {
        el.classList.remove('active');
      });

      // 给目标节点添加 active 类，这样 sidebar-collapse-plugin 就能找到它
      liNode.classList.add('active');
      console.log('[Search] Added active class to target node, sidebar-collapse-plugin should handle the rest');

      // 延迟执行额外的展开逻辑，确保所有父级都被展开
      setTimeout(function() {
        console.log('[Search] Double-checking expansion...');
        liNode.classList.add('open');
        liNode.classList.remove('collapse');
        ...
        while (node && node.parentNode) {
          ...
          if (
            ...
            (node.parentNode.classList && node.parentNode.classList.contains('app-sub-sidebar'))
          ) {
            console.log('[Search] Opening parent', parentCount++, ':', node.parentNode.tagName, node.parentNode.className);
            node.parentNode.classList.add('open');
            node.parentNode.classList.remove('collapse');
          }
          node = node.parentNode;
        }
        console.log('[Search] Opened', parentCount, 'parent nodes');
      }, 500);
    } else {
      console.log('[Search] Could not find LI node for link');
    }
  }
  ...
  function bindEvents() {
    ...
    Docsify.dom.on(document, 'click', function (e) {
      var target = e.target;
      // 查找是否点击了搜索结果链接
      while (target && target.tagName !== 'A') {
        target = target.parentNode;
      }
      if (target && target.classList && target.classList.contains('search-result-link')) {
        // 标记需要在导航完成后展开侧边栏
        expandAfterNavigation = true;
      }
    });
  }
